...
class AdaptiveProductionPreprocessor(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if self.numeric_cols_ and self.scaler_ is not None:
            output_parts.append(
                self.scaler_.transform(X_df[self.numeric_cols_])
            )

        if self.categorical_cols_ and self.encoder_ is not None:
            output_parts.append(
                self.encoder_.transform(X_df[self.categorical_cols_])
            )
        ...
        for col in self.datetime_cols_:
            ...
            h_sin = np.sin(
                2 * np.pi * dt_series.dt.hour / 24.0
            ).values.reshape(-1, 1)
            ...
            m_sin = np.sin(
                2 * np.pi * dt_series.dt.month / 12.0
            ).values.reshape(-1, 1)
...
with tab1:
    ...
    with r2_c1:
        ...
        target_temp_min = st.number_input(
            "Minimum Safe Temp (°C)",
            value=float(limits["min_temp"]),
            step=0.1
        )

    with r2_c2:

        target_temp_max = st.number_input(
            "Maximum Safe Temp (°C)",
            value=float(limits["max_temp"]),
            step=0.1
        )
    ...
    with r3_c1:
        ...
        door_open_count = st.number_input(
            "Number of Times Door Opened (Last 1 Hour)",
            min_value=0,
            value=9,
            step=1
        )
    ...
    with r3_c2:
        ...
        door_open_duration = st.number_input(
            "Total Seconds Door Stayed Open (Last 1 Hour)",
            min_value=0,
            value=283,
            step=10
        )
    ...
    with r5_c1:
        ...
        compressor_duty_cycle = st.slider(
            "Compressor Workload Ratio",
            min_value=0.0,
            max_value=1.0,
            value=0.98,
            step=0.01
    ...
    with r5_c2:
        ...
        defrost_cycle_active = st.selectbox(
            "Is Defrost Cycle Running Right Now?",
            options=[0, 1],
    ...
    if st.button(
        ...
        time_string = (
            f"{date_input.strftime('%d:%m:%Y')}, "
            f"{time_input.strftime('%H:%M')}"
        )

        if (
            current_internal_temp < target_temp_min
            or
            current_internal_temp > target_temp_max
        ):
            ...
            st.metric(
                ...
                value="1.00 (Maximum Risk)"
            )

            st.warning(
                "Action Required: The temperature inside "
                "the container has already gone past "
                "the safe limits."
            )
            ...
            input_row = pd.DataFrame({
                ...
                'cumulative_door_open_duration_1h': [
                    door_open_duration
                ],
                ...
                'compressor_duty_cycle_ratio_2h': [
                ...
                'cargo_volume_utilization_ratio': [
                    cargo_utilization
                ]
            })
            ...
            try:
                ...
                st.metric(
                    label="Calculated Risk Score",
                    value=f"{final_risk:.2f}"
                )

                if final_risk >= 0.80:

                    st.error(
                        "HIGH RISK: This container is highly "
                    ...
                    st.warning(
                        "MODERATE RISK: Risk is elevated."
                    )

                else:

                    st.success(
                        "SAFE: This container is operating "
                        "normally."
                    )
                ...
                st.error(
                    f"Calculation Error: {str(error_msg)}"
                )
...
with tab2:
    ...
    st.subheader(
        "High-Risk Shipment Diagnostics (Risk > 80%)"
    )

    dataset_start_date = (
        historical_data['parsed_time']
        .min()
        .date()
    )
    ...
    with sub_col1:
        ...
        shc_options = sorted(
            historical_data['SHC_code']
            .unique()
            .tolist()
        )

        selected_shc = st.multiselect(
            "Filter Cargo Types",
            options=shc_options,
            default=shc_options
        )
    ...
    with sub_col2:
        ...
        weather_options = sorted(
            historical_data['weather_condition']
            .unique()
            .tolist()
        )
    ...
    with stat2:
        ...
        avg_risk = (
            filtered_data['excursion_risk_score']
            .mean()
    ...
    with stat3:
        ...
        avg_temp = (
            filtered_data['current_internal_temp']
            .mean()
            if not filtered_data.empty
            else 0.0
        )
        ...
        st.metric(
            "Avg Container Temp during Alert",
    ...
    if not filtered_data.empty:
        ...
        with chart_row1_left:
            ...
            st.markdown(
                "### High-Risk Incidents by Cooling System Age"
            )

            machinery_age_data = (
                filtered_data.groupby(
                    pd.cut(
                        filtered_data['equipment_age_years'],
                        bins=[0, 2, 5, 8, 12, 20],
                        labels=[
                            "0-2 Yrs",
                            "2-5 Yrs",
            ...
            fig_age = px.bar(
                ...
                text_auto=True,
                labels={
                    "equipment_age_years": "Cooling System Age",
                    "Incident Count": "Number of High-Risk Incidents"
                },
        ...
        with chart_row1_right:
            ...
            st.markdown(
                "### Total High-Risk Incidents by Cargo Type"
            )

            shc_summary = (
                filtered_data.groupby('SHC_code')
                .size()
                .reset_index(name='Incident Count')
            )
        ...
        with chart_row2_left:
            ...
            st.markdown(
                "### Total High-Risk Incidents by Weather Condition"
            )

            weather_summary = (
                filtered_data.groupby('weather_condition')
                .size()
                .reset_index(name='Incident Count')
            )
        ...
        with chart_row2_right:
            ...
            st.markdown(
                "### Share of Critical Incidents by Weather Status"
            )
        ...
        st.info(
            "No shipments fall into the critical category "
            "(> 80% Risk) matching the selected filters."
        )
